...
class EvidenceLineageValidatorTests(unittest.TestCase):
    ...
    def test_missing_source_provenance_denies(self):
        evidence = self.base(); del evidence["observation_origin"]
    ...
    def test_derived_source_truth_masquerade_denies(self):
        result = validate_evidence(self.base() | {"authority": "source_truth", "derived": True})
        self.assertEqual(result["status"], "DENY")
    ...
    def test_local_receipt_cannot_claim_external_independence(self):
        result = validate_evidence(self.base() | {"observation_origin": "local_receipt", "independent_external": True})
        self.assertEqual(result["status"], "DENY")
    ...
    def test_derived_evidence_requires_upstream_lineage(self):
        result = validate_evidence(self.base() | {"derived": True})
        self.assertEqual(result["status"], "DENY")
    ...
    def test_runtime_admission_requires_runtime_identity_and_gate_evidence(self):
        result = validate_evidence(self.base() | {"runtime_admission": True})
        self.assertEqual(result["status"], "DENY")
    ...
    def test_canonical_promotion_requires_payload_hash(self):
        result = validate_evidence(self.base() | {"promoted_canonical": True})
        self.assertEqual(result["status"], "DENY")
    ...
    def test_same_value_for_raw_and_semantic_hash_denies_without_explicit_distinction(self):
        evidence = self.base() | {"raw_artifact_sha256": "same", "semantic_fingerprint": "same"}
        result = validate_evidence(evidence)
        self.assertEqual(result["status"], "DENY")
        self.assertEqual(result["reason"], "RAW_AND_SEMANTIC_HASH_CONFLATED")

    def test_semantic_truth_without_domain_denies(self):
        evidence = self.base() | {"semantic_fingerprint": "meaning", "semantic_truth_asserted": True}
        result = validate_evidence(evidence)
        self.assertEqual(result["status"], "DENY")
        self.assertEqual(result["reason"], "SEMANTIC_HASH_REQUIRES_VALIDATED_DOMAIN")

    def test_legacy_alias_requires_explicit_fixture(self):
        result = validate_evidence(self.base() | {"raw_artifact_exists": True, "raw_sha256": "legacy-raw"})
        self.assertEqual(result["status"], "DENY")
    ...
    def test_legacy_fixture_can_be_read_without_becoming_canonical(self):
        evidence = self.base() | {"raw_artifact_exists": True, "raw_sha256": "legacy-raw", "legacy_fixture": True}
        self.assertEqual(validate_evidence(evidence)["status"], "PASS")

    def test_canonical_field_wins_over_legacy_alias(self):
        evidence = self.base() | {"raw_artifact_exists": True, "raw_artifact_sha256": "canonical", "raw_sha256": "legacy"}
        self.assertEqual(validate_evidence(evidence)["status"], "PASS")

    def test_validator_does_not_mutate_evidence(self):
        evidence = self.base() | {"derived": True, "upstream_evidence_ids": ["E1"], "derivation_contract": "DERIVATION_V1"}
