...
def _semantic_errors(state: dict) -> list[str]:
    ...
    if state.get("forensic_fsm") != "ONE_FORENSIC_FSM": errors.append("forensic FSM must be ONE_FORENSIC_FSM")
...
def reconcile(runtime_commit: str | None = None, deployment_id: str | None = None) -> dict:
    ...
    deploy_evidence = {"known": bool(observed_deploy),"expected_last_verified": expected_deploy or "UNKNOWN","observed": observed_deploy or "UNKNOWN","identity_rule":"DEPLOYMENT_ID_IS_EVIDENCE_ONLY"}
    ...
    if status == "VERIFIED" and runtime_known and not runtime_baseline_known: status = "RECONCILE_REQUIRED"
    elif status == "VERIFIED" and runtime_known and runtime_baseline_known and not runtime_is_same_as_last_verified: status = "RECONCILE_REQUIRED"
    return {"state_consistency":status,"authority":"BRAIN_CURRENT_STATE_ONLY" if authority_ok else "DENY","authority_protocol_version":protocol,"protocol_compatible":protocol_compatible,"state_schema_version":schema_version,"state_schema_known":schema_known,"state_mode":state.get("state_mode"),"state":state.get("state"),"promotion":state.get("promotion"),"action_space":state.get("action_space"),"action":state.get("action"),"semantic_errors":semantic_errors,"runtime_commit_known":runtime_known,"runtime_commit":observed_runtime or "UNKNOWN","runtime_baseline_known":runtime_baseline_known,"runtime_last_verified_commit":expected_runtime or "UNKNOWN","runtime_commit_same_as_last_verified":runtime_is_same_as_last_verified,"deployment_evidence":deploy_evidence,"runtime_is_authority":False,"downstream_override_allowed":False,"unknown_is_not_pass":True,"default_deny":True,"brain_state_blob_sha":state_outer.get("sha", "UNKNOWN")}
