...
This module deliberately stores compact forensic envelopes only. It never owns
source data and never loads FULL_27/TAIL_27 or any bulk dataset.
"""
...
def _require_tls_database_url(url: str) -> str:
    ...
    # Never return or log credentials. The normalized URL is passed only to the
    # database driver and always carries an explicit TLS mode.
...
def record_envelope(envelope: dict[str, object], *, database_url: str | None = None) -> EvidenceReceipt:
    """Insert one compact evidence envelope and return its receipt.

    The caller must provide only metadata. The payload is hashed before the
    database write and the same hash is stored with it. Credentials never enter
    the envelope.
    """
...
def verify_receipt(receipt: EvidenceReceipt, *, database_url: str | None = None) -> bool:
    """Read one receipt back and verify the stored envelope hash exactly."""
    url = _require_tls_database_url(database_url or os.environ.get("DATABASE_URL", ""))
    psycopg = _load_psycopg()
    try:
        with psycopg.connect(url, connect_timeout=5) as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT envelope_sha, envelope_json FROM brain_forensic_evidence WHERE evidence_id=%s",
                    (receipt.evidence_id,),
                )
                row = cur.fetchone()
    except Exception as exc:
