...
The database chain is deliberately explicit:
DB_EXISTENCE -> DB_BINDING -> DB_TLS_ADMISSION -> NETWORK_ORIGIN_PROOF
-> DB_ROUND_TRIP -> PROMOTION.
"""
...
def promote(evidence_chain: tuple[GateEvidence, ...]) -> ForensicDecision:
    """Promotion requires independent evidence for every infrastructure proposition."""
    required = (
        Gate.EXISTENCE,
        Gate.BINDING,
        Gate.SECURITY,
    ...
    return ForensicDecision(
        GateStatus.PASS,
        Gate.PROMOTION,
        "ALL_REQUIRED_GATES_INDEPENDENTLY_PROVEN",
    )
