...
def poll():
 global LAST,RESULT
 try:
  raw=get('coordination/worker_allocation_v1.json'); sha=hashlib.sha256(raw.encode()).hexdigest(); outer=json.loads(raw); alloc=json.loads(outer['content']) if isinstance(outer.get('content'),str) else outer; task=alloc.get('workers',{}).get('BOT4_EXECUTION',{}); nxt=json.loads(get('state/next_action.json'))
  ...
  RESULT={"schema":"headless-worker-result/v1","result_type":"EXECUTION_RECEIPT","worker":"BOT4_EXECUTION","allocation_id":alloc.get('allocation_id'),"cycle_id":alloc.get('cycle_id'),"allocation_sha256":sha,"checks":checks,"result":"PASS" if all(checks.values()) else "HOLD","promotion":"DENY","canonical_mutation":"FORBIDDEN","observed_at":datetime.now(timezone.utc).isoformat()}
  print(json.dumps(RESULT,sort_keys=True),flush=True)
 except Exception as e:
  RESULT={"schema":"headless-worker-result/v1","result_type":"EXECUTION_RECEIPT","worker":"BOT4_EXECUTION","result":"HOLD","error":type(e).__name__,"promotion":"DENY","canonical_mutation":"FORBIDDEN","observed_at":datetime.now(timezone.utc).isoformat()}; print(json.dumps(RESULT,sort_keys=True),flush=True)
...
if __name__=='__main__':
 s=ThreadingHTTPServer(('0.0.0.0',PORT),H); t=threading.Thread(target=lambda: [poll() or time.sleep(POLL) for _ in iter(int,1)],daemon=True); t.start(); print(json.dumps({"status":"HTTP_READY","worker":"BOT4_EXECUTION","port":PORT}),flush=True); s.serve_forever()
