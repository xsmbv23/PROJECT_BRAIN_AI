...
def cycle():
 raw=get_raw('coordination/worker_allocation_v1.json'); outer=json.loads(raw); return json.loads(outer['content']) if isinstance(outer.get('content'),str) else outer
def tick():
 global LAST
 try:
  alloc=cycle(); h2=health(BOT2); h4=health(BOT4); reachable=(h2.get('status')=='ALLOCATION_OBSERVED' and h4.get('status')=='ALLOCATION_OBSERVED')
...
if __name__=='__main__':
 ...
 t=threading.Thread(target=lambda: [tick() or time.sleep(POLL) for _ in iter(int,1)],daemon=True); t.start()
 print(json.dumps({"status":"HTTP_READY","component":"HEADLESS_SUPERVISOR","port":PORT}),flush=True)
 s.serve_forever()
