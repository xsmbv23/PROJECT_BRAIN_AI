#!/usr/bin/env python3
"""Headless supervisor: compact-context-first worker health/result reconciliation."""
...
def alloc():
 ...
 return a,active,sha
def context():
 r=raw('coordination/BOT_OPERATING_CONTEXT_V1.json'); return json.loads(r),hashlib.sha256(r.encode()).hexdigest()
def prior():
 try:
...
def tick():
 global LAST,RECEIPT
 ctx,csha=context(); a,active,ash=alloc(); obs={}
 for wid in a.get('workers',{}):
  if wid not in active: obs[wid]={'status':'PAUSED','execution_required':False}; continue
 ...
 result_ok=configured and all(obs[wid]['result'].get('result') in ('PASS','HOLD') for wid in active)
 ...
 next_action='BOT1_RECONCILE_AND_ALLOCATE_NEXT' if overall=='PASS' else 'HOLD_AND_DIAGNOSE_WORKER_PATH'
 r={'schema':'headless-reconciliation-receipt/v8','receipt_type':'WORKER_RECONCILIATION','issued_at':now,'context_sha256':csha,'allocation_sha256':ash,'allocation_id':a.get('allocation_id'),'cycle_id':a.get('cycle_id'),'active_workers':active,'workers':obs,'checks':{'compact_context':bool(ctx.get('schema')),'active_workers_configured':configured,'health':health_ok,'results':result_ok,'allocation_identity':identity_ok,'prior_receipt_anchor':prior_ok},'result':overall,'next_action':next_action,'canonical_mutation':'BOT1_ONLY','promotion':'DENY','chat_session_execution':'CLOSED','execution_authority':'HEADLESS_WORKER'}
 r['receipt_sha256']=hashlib.sha256(json.dumps(r,sort_keys=True,separators=(',',':')).encode()).hexdigest(); RECEIPT=r; LAST={'schema':'headless-orchestrator/v9','observed_at':now,'allocation_id':a.get('allocation_id'),'cycle_id':a.get('cycle_id'),'active_workers':active,'workers':obs,'health_status':'PASS' if health_ok else 'HOLD','result_status':overall,'next_action':next_action,'promotion':'DENY','chat_session_execution':'CLOSED'}; print(json.dumps(LAST,sort_keys=True),flush=True)
